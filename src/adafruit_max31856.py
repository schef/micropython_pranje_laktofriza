...
class MAX31856:
    ...
    def __init__(self, spi, cs, thermocouple_type = ThermocoupleType.K, baudrate = 500000):
    ...
    def __init__(self, spi, cs_pin, thermocouple_type=0x03, baudrate=500000):
        self._spi = spi
        self._cs = Pin(cs_pin, Pin.OUT)
        self._cs.value(1)  # Ensure CS is high initially
    ...
    def _set_thermocouple_type(self, thermocouple_type):
        conf_reg_1 = self._read_register(_MAX31856_CR1_REG, 1)[0]
        conf_reg_1 &= 0xF0  # Mask off lower 4 bits
        conf_reg_1 |= thermocouple_type & 0x0F
    ...
    def _read_temperature(self):
        raw_temp = ustruct.unpack(">i", self._read_register(_MAX31856_LTCBH_REG, 3) + b"\x00")[0]
        raw_temp >>= 8
        return raw_temp / 4096.0

    def _perform_one_shot_measurement(self):
        self._write_u8(_MAX31856_CR0_REG, _MAX31856_CR0_1SHOT)
        while self._read_register(_MAX31856_CR0_REG, 1)[0] & _MAX31856_CR0_1SHOT:
            sleep(0.01)
    ...
    def _write_u8(self, address, value):
        self._cs.value(0)
        self._spi.write(bytearray([(address | 0x80) & 0xFF, value & 0xFF]))
        self._cs.value(1)
